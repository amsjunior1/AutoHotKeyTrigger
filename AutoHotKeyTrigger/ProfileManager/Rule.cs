...
namespace AutoHotKeyTrigger.ProfileManager
{
    ...
    public class Rule
    {
        ...
        // Original properties from the base file
        ...
        /// </summary>
        /// <param name="other">The other rule to clone.</param>
        ...
        public void Clear()
        {
            ...
            this.AdvancedConditionScript = string.Empty;
        ...
        /// <summary>
        ///     Checks the rule conditions and executes its action if conditions are satisfied.
        /// </summary>
        /// <param name="logger">Action to log messages.</param>
        /// <param name="vcm">Virtual Controller Manager from the Core.</param>
                ...
                switch (this.TypeOfAction)
                {
                    ...
                    case ActionType.Controller:
                        if (this.ControllerButton != null)
                        {
                            if (vcm != null)
                            {
                                vcm.InjectPress(this.ControllerButton);
                                actionTaken = true;
                                actionMessage = $"---> SUCCESS: Injected VIRTUAL button {this.ControllerButton.Name}.";
                            }
                        }
                        break;
                }
        ...
        private bool Evaluate()
        {
            if (this.cooldownStopwatch.Elapsed.TotalSeconds > this.delayBetweenRuns)
            {
                ...
                {
                    this.AdvancedConditionScript = string.Join(" && ", this.oldConditions.Select(c => c.ToString()));
